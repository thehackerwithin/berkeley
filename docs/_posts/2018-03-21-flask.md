...
applications. A web app runs in a browser. The web server be run locally on
your laptop, or it can be on a remove server. Making a Flask app is
...
"D3" style plots using a imperative style like Matplotlib (versus a declarative
style like [Altair](https://altair-viz.github.io/)). Bokeh is ideally suited
for embedding plots in a web app like Flask. Let's see how you can add a Bokeh
plot to your `hello.py` Flask app.
...
style sheets (CSS) which provide the custom HTML and 3 JavaScript scripts that
...
find yourself dabbing in CSS and JS too. Embrace it.
