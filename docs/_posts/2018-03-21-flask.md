...
misinformation - avoid [W3Schools](https://en.wikipedia.org/wiki/W3Schools) and
go straight to the horses mouth. Mozilla invented the internet, not Al Gore, so
when if you need to find out anything about HTML, CSS, or JS, always check the
[Mozilla Developer Network (MDN)](https://developer.mozilla.org/en-US/) first!
